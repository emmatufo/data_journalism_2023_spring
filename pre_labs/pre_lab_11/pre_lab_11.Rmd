...
**Answer Now you can see the press releases from this year and it's an actual dataframe** 
...
**Answer cleans the names to make them lowercase and renames news_release_news_item to title and changes date column to an actual date** 
...
html element grabs first instance of tag
html elements grabs all
put tag itself in quotes

**Answer grabs all li items** 
...
**Answer gets the text inside the tag - easier to read that the html bc you see what's in there at each line** 
...
**Answer turns it into a dataframe** 
...
**Task ** Run the following code in the gray-colored codeblock below to remove the unwanted rows from our dataframe:isolates it for just the listing item ones to be shown, no longer menu items BUT THE DATE AND TITLE ARE IN ONE COLUMN
...
**Task** Run the following code in the gray-colored codeblock below to split the `value` column into two columns based on the ":" character and name them: SEPARATES THE DATE AND TITLE --- everything on left side of colon goes left column, right side goes right
...
**Task** Run the following code in the gray-colored codeblock below to get rid of releases without dates and trim the leading spaces. Just for fun, we'll convert the dates into actual dates, too: gets rid of na, gets rid of leading space before titles, covert date to actual date datatype
