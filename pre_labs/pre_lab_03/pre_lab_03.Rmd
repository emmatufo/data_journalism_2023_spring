...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer No, there are no longer problems. tHE ZIPCODE NOW APPEARS AS A CHARACTER COLUMN - NO LONGER JUST NUMERIC**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answerdidn't have to do*
...
**Answer 547 - somebody produced this file , opened it in excel and saved it, geting rid of the 0s before some of the zips**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer some have N/A for category. Some have just 'L' WHICH WOULD NEED TO BE CONFIRMED as loans. Then, there's 'contract' which would need to be figured out in terms of definition**
...
**Answer Amount in ascending order**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer the headers are lowercase, spaces have underscores**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer 17740**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer There is a new column called zip5 with the 5 digit zipcodes**
