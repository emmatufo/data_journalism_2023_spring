...
**Answer states, percentage of the state that voted for trump, percentage of the businesses receiving the ppp loan and ppp amount per employee**
```{r}
...
**Answer there is a mostly positive correlation, but there's a bunch of outliers - they do seem to be related between percentage of businesses that got a loan and percentage of that state that voted for trump**
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer the blue line is much more flat, so there's not much of a relationship here between pct of state that voted for trump and ppp amount per employee**
```{r}
...
**Answer the actual correlation pct, how much they're related. tells how much of a positive reltionship: .5 --> more positive than .2 **
...
**Answer how valid is the relationship --> the low p value means it's probably real**
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer there's a higher p value here, showing it's likely the finding is not statistically valid**
```{r}
...
**Answer dataframe showing the average ppp for all states won by clinton and trump, with the percentage of businesses that actually got them. trump states did better**
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer there's a wide range in confidence interval, it's statistically insignificant**
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer here, it's staistically sound with less wide of a range**
```{r}
